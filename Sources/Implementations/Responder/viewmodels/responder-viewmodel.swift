...
public class ResponderViewModel: ObservableObject {
    ...
    private struct PaymentLinkEnvelope: Decodable {
        let success: Bool
            ...
            DispatchQueue.main.async {
                ...
                case .success(let data):
                    ...
                    if self.apiPathVm.selectedRoute == .template {
                        ...
                        self.bannerColor = .green
                        self.successBannerMessage = "Operation completed successfully"

                        if let env = try? JSONDecoder().decode(
                            PaymentLinkEnvelope.self, from: data),
                            env.success,
                            let link = env.payment_link,
                            !link.isEmpty
                        {
                            link.scp()
                            self.successBannerMessage = "Payment link copied to clipboard."
                        }
